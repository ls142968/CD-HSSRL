...
bool SubseaPressureROSPlugin::OnUpdate(const common::UpdateInfo& _info)
{
  ...
  double depth = pos.Z();//std::abs(pos.Z());
  double pressure = 0;   //this->standardPressure;
  if (depth >= 0)
  {
	  pressure = 0;//this->standardPressure;
    // Convert depth to pressure
    //pressure += depth * this->kPaPerM;
  }
  else
  {
	  pressure += std::abs(depth);// * this->kPaPerM;
  }
  ...
  {
    inferredDepth = pressure;//(pressure - this->standardPressure) / this->kPaPerM;
  }
